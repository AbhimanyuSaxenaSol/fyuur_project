...
# Models.
#----------------------------------------------------------------------------#
# TODO def past_shows(id):

def past_shows(id):
  shows= Shows.query.filter_by(venue_id=id)
  past=[]
  for show in shows:
...
# TODO Implement Show and Artist models, and complete all model relationships and properties, as a database migration//attempeted not completed
...
def venues():
  ...
  data = Venue.query.with_entities(Venue.city, Venue.state, postgresql.array_agg(func.json_build_object('id',Venue.id,'name',Venue.name)).label('venues'))\
...
def create_artist_submission():
  # called upon submittin  g the new artist listing form
  # TODO: insert form data as a new Venue record in the db, instead
  # TODO: modify data to be the data object returned from db insertion
  error = False
  try:
...
def shows():
  ...
  for show in shows:
    artist = Artist.query.filter_by(id=show.artist_id).first()
    venue = Venue.query.filter_by(id=show.venue_id).first()
    info =  {"venue_id": show.venue_id, "venue_name": venue.name, "artist-id": show.artist_id, "artist_name": artist.name, "artist_image_link":artist.image_link, "start_time":str(show.start_time)}
    data.append(info)
